...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        W2 = np.random.randn(H//2 * W//2 * num_filters,
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        loss, daffine2_out = softmax_loss(scores, y)
        loss += 0.5 * self.reg * (np.sum(W1 * W1) +
                                  np.sum(W2 * W2) + np.sum(W3 * W3))
        daffine1_out, dW3, db3 = affine_backward(daffine2_out, affine2_cache)
        dW3 += self.reg * W3
        dconv_out, dW2, db2 = affine_relu_backward(daffine1_out, affine1_cache)
        dW2 += self.reg * W2
